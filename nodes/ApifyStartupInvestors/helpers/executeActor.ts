...
import { ACTOR_ID } from '../ApifyStartupInvestors.node';
import { buildActorInput } from '../ApifyStartupInvestors.properties';
...
/**
 * Shape a single investor firm according to the chosen Output mode.
...
function shapeItem(
	...
	return {
		...
		website: item.firm_website,
		crunchbaseUrl: item.crunchbase_url,
		description: item.firm_description,
		focus: item.firm_focus,
		stages: item.firm_stages,
		aum: item.firm_aum,
		firmId: item.firm_id,
	};
}
